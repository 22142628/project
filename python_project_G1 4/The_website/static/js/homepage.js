    ...
    const trails = [
        {
            ...
            type: "out-and-back",
            rating: 4.5,
            description: "Rail trails follow the routes of former railway lines, offering scenic routes through diverse landscapes. This trail takes you through beautiful countryside with historic railway stations along the way.",
            ...
            comments: [
                {
                    ...
                    rating: 4,
                    text: "This trail has amazing views, especially in the morning! The surface is well-maintained and suitable for all skill levels."
                },
                {
                    author: "TrailMaster",
                    rating: 5,
                    text: "Great for beginners and families. Well maintained with plenty of spots to stop for picnics. The historical markers add interesting context to the journey."
                }
            ]
        },
        {
            ...
            type: "loop",
            rating: 4.2,
            description: "Experience the beauty of Victoria's high country on this popular trail with gentle gradients. Perfect for cyclists and walkers alike, with charming towns along the route.",
            ...
            comments: [
                {
                    ...
                    rating: 5,
                    text: "The wildflowers along this trail in spring are breathtaking! The trail is well-marked and has excellent facilities at regular intervals."
                }
            ]
        },
        {
            ...
            type: "point-to-point",
            rating: 4.0,
            description: "Follow this coastal trail for spectacular ocean views and access to beautiful beaches. A shorter trail perfect for a day trip with family.",
            image: "https://images.unsplash.com/photo-1452421822248-d4c2b47f0c81?ixlib=rb-1.2.1&auto=format&fit=crop&w=600&q=80",
            comments: [
                {
                    author: "BeachWalker",
                    rating: 4,
                    text: "Perfect for a summer day - bring your swimsuit! The trail has several access points to pristine beaches that are less crowded than the main tourist spots."
                },
                {
                    author: "PhotographyFan",
                    rating: 5,
                    text: "Sunset views are incredible here. Don't forget your camera. The trail offers multiple vantage points for stunning coastal photography."
                }
            ]
        }
    ];
...
// ✅ Popup for live search match
function showTrailPopup(trail) {
    const popup = document.createElement('div');
    popup.className = 'popup';
    popup.style.cssText = 'position: fixed; top: 20%; left: 50%; transform: translateX(-50%); background: white; padding: 2rem; border: 2px solid #2c7744; z-index: 1000; max-width: 400px; box-shadow: 0 8px 16px rgba(0,0,0,0.2);';

    popup.innerHTML = `
        <h3>${trail.trail_name || trail.name} - ${trail.location}</h3>
        <p><strong>Distance:</strong> ${trail.distance} km</p>
        <p><strong>Difficulty:</strong> ${trail.difficulty}</p>
        <p><strong>Estimated Time:</strong> ${trail.estimated_time || 'N/A'} hrs</p>
        <p><strong>Notes:</strong> ${trail.notes || trail.description || 'None'}</p>
...
document.getElementById('main-search-form').addEventListener('submit', function(e) {
    e.preventDefault();
    const query = document.getElementById('trail-name').value.trim().toLowerCase();
    const match = trails.find(t => (t.trail_name || t.name).toLowerCase().includes(query));
    if (match) {
        showTrailPopup(match);
    } else {
        alert("No trail found.");
    }
});
